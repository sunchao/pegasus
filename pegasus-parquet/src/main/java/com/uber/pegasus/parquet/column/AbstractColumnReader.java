...
import com.uber.pegasus.parquet.value.PlainBinaryValuesReader;
...
public abstract class AbstractColumnReader<V extends FieldVector> {
  ...
  public void readBatch(int total, V column) {
    ...
    while (total > 0) {
      ...
      column.setValueCount(column.getValueCount() + valuesInCurrentPage);
    }
  }
  ...
  private static <V extends ValueVector> ValuesReader<V> getPlainValuesReader(
    ...
    switch (desc.getPrimitiveType().getPrimitiveTypeName()) {
      ...
      case BINARY:
        return (ValuesReader<V>) new PlainBinaryValuesReader();
      default:
        throw new UnsupportedOperationException(
